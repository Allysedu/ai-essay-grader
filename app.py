...
if st.button("🚀 모든 파일 평가 시작"):
    if not st.session_state.google_api_key or not eval_name:
        st.error("⚠️ API 키와 평가명을 모두 입력해주세요!")
    elif not uploaded_essays:
        st.warning("⚠️ 평가할 에세이 파일을 먼저 업로드해주세요.")
    else:
        results = []
        ...
        with st.expander(f"📄 {result['파일명']} 상세 결과 보기"):
            ...
            if parsed_data and '항목별 평가' in parsed_data:
                ...
                for item_name, details in itemized_scores.items():
                    ...
                    st.markdown(f"> {details.get('이유', '내용 없음')}") # 들여쓰기된 인용구
            else:
